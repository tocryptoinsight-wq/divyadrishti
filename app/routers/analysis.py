...
async def get_screener(
    ...
    async def _process_symbol(sym):
        ...
        try:
            data_1h, data_15m, data_5m = await asyncio.gather(
                delta_client.get_candles(sym, "1h", limit=500),
                delta_client.get_candles(sym, "15m", limit=500),
                delta_client.get_candles(sym, "5m", limit=500),
            )
...
async def get_ticker_price(symbol: str):
    ...
    async def _fetch_ticker():
        try:
            ticker = await delta_client.get_ticker(symbol)
            raw = ticker.get("result", ticker)
            return float(raw.get("mark_price", raw.get("close", raw.get("last", 0))))
        except Exception as e:
    ...
    async def _fetch_candle():
        try:
            ...
            return float(close_arr[-1]) if len(close_arr) > 0 else 0
        except Exception as e:
            logger.warning("Failed to fetch candle close: %s", e)
            return 0
    last_price, last_close = await asyncio.gather(_fetch_ticker(), _fetch_candle())
